    ...
    <header class="navbar sticky-top flex-md-nowrap p-0 shadow" style="background-color: #7AA874">
      ...
      <div class="navbar-nav">
        <div class="nav-item text-nowrap">
          <a class="nav-link px-3" style="color: white" href="{{ route('siswa') }}">Kembali</a>
        </div>
      </div>
    </header>
<main>
  ...
  <div class="container d-flex justify-content-center mt-3">
      <div class="container mt-3">
        <div class="row">
            <div class="col">
                <h3 class="text-center">Edit Data Siswa</h3><br>
                <div class="container ">
                        ...
                        <div class="col-md-5">
                            <form action="{{ route('siswa.update', $siswa->id_siswa) }}" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="nama_input" name="namaInput" value="{{ $siswa->nama }}">
                                </div>
                                <div class="mb-3">
                                    <label for="nis_input" class="form-label">NIS</label>
                                    <input type="text" class="form-control" id="nis_input" name="nisInput" value="{{ $siswa->nis }}">
                                </div>
                                <div class="mb-3">
                                    <label for="kelas_input" class="form-label">Kelas</label>
                                    <input type="text" class="form-control" id="kelas_input" name="kelasInput" value="{{ $siswa->kelas }}">
                                </div>
                                <div class="mb-3">
                                    <label for="jabatan_input" class="form-label">Jabatan</label>
                                    <input type="text" class="form-control" id="jabatan_input" name="jabatanInput" value="{{ $siswa->jabatan }}">
                                </div>
                                <div class="mb-3">
                                  <select class="form-select form-select-sm" aria-label=".form-select-sm example" name="ekstraInput" id="ekstraInput">
                                    ...
                                    <option value="{{$user->id_ekstra}}" {{$user->id_ekstra == $siswa->id_ekstra ? 'selected' : ''}}>{{$user->nama_ekstra}}</option>
                                    @endforeach
                                  </select>
                              </div>
                                <div class="row mx-2">

      ...
      <div class="my-3 col-12 col-sm-8 col-md-5">
        <form action="" method="get">
          <div class="input-group mb-3">
            <input type="text" class="form-control" placeholder="Cari berdasarkan nama" name="search" aria-describedby="basic-addon1">
          ...
          <tbody>
            ...
            <tr>
                <th scope="row">{{ $n }}</th>
                    <td>{{ $user->name }}</td>
                    <td>{{ $user->email }}</td>
                    <td>{{ ($user->is_admin)== 1 ? "Admin" : "Ketua Ekstra" }}</td>
                    <td>
                      <td>
                        <div style="display: flex; gap: 10px;">
                            <a href="{{ route('dashboard.edit', encrypt($user->id)) }}" class="btn btn-primary">Edit</a>
